...
* More recently, the code in `cpu/report.py` and `gpu/report.py` has been run directly, e.g., via
```
python -m report
```
and output copied into `notes.txt`. 

* `cpu/report.py` and `gpu/report.py` are currently identical, produced by
```
make report.py
```
in the `cpu` directory, which in turn is extracted from `cpu/report.qmd`
