...
import pandas as pd
import pypomp as pp
from pypomp.spx import spx
import jax
```

```{python Load data in python}
PKL_PATH = "cpu/spx_results.pkl"
...
LL_frame = (
    pd.DataFrame(
        {
            "LL": [float(x.mean()) for x in pkl_in.results[-1]["logLiks"]],
            "sd": [float(x.std()) for x in pkl_in.results[-1]["logLiks"]],
        }
    )
...
float(spx_obj.results[-1]["logLiks"][0].mean())
float(spx_obj.results[-1]["logLiks"][0].std())
```
