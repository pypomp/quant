...
dataset.index = (
    pd.read_csv("data/dacca/covart.csv", index_col=0).reset_index(drop=True).squeeze()
)
dataset = dataset.reindex(
    onp.array([1891 + i * (1 / 240) for i in range(12037)])
).interpolate()
...
for i in range(Nfilt):
    ...
    logliks.append(
        pfilter(theta, ys, J, covars, thresh=-1, key=jax.random.PRNGKey(42 + i))
    )
