...
def transform_to_perturbation_scale(df):
    res = df.copy()
    # Log transforms
    for col in ["R0", "sigma", "gamma", "iota", "sigmaSE", "psi"]:
        if col in res.columns:
            res[col] = np.log(res[col])
    
    # Logit transforms
    for col in ["cohort", "amplitude", "rho"]:
        if col in res.columns:
            res[col] = logit(res[col].clip(1e-7, 1-1e-7))
            
    # IVPs (log-fractions)
    ivp_cols = ["S_0", "E_0", "I_0", "R_0"]
    if all(c in res.columns for c in ivp_cols):
        sum_ivp = res[ivp_cols].sum(axis=1)
        for c in ivp_cols:
            res[c] = np.log(res[c] / sum_ivp)
            
    return res

# Apply to IF2
traces_pert = transform_to_perturbation_scale(traces)
traces_long_pert = traces_pert.melt(
    id_vars=id_cols,
    value_vars=param_cols,
    var_name="quantity",
    value_name="param_value",
...
# Apply to IFAD
traces_pert_ifad = transform_to_perturbation_scale(traces_ifad)
traces_long_pert_ifad = traces_pert_ifad.melt(
    id_vars=id_cols_ifad,
    value_vars=param_cols_ifad,
    var_name="quantity",
    value_name="param_value",
...
## Parameter traces (perturbation scale)

### IF2

```{python}
# | label: plot-param-traces-pert
# | fig-width: 12
# | fig-height: 12
# | echo: false
(
    ...
    + geom_line(show_legend=False)
    + facet_wrap("quantity", scales="free_y")
    + labs(title="Parameter traces (perturbation scale - IF2)")
)
```
...
```{python}
# | label: plot-param-traces-ifad-pert
# | fig-width: 12
# | fig-height: 12
# | echo: false
(
    ggplot(
        traces_long_pert_ifad,
        aes(x="iter", y="param_value", group="rep", color="factor(rep)"),
    )
    + geom_line(show_legend=False)
    + facet_wrap("quantity", scales="free_y")
    + labs(title="Parameter traces (perturbation scale - IFAD)")
)
```
...
# Extract initial train logLik
train_traces_ifad = traces_ifad[traces_ifad["method"] == "train"]
first_train_iter_ifad = train_traces_ifad["iter"].min()
initial_loglik_ifad = train_traces_ifad.loc[
    train_traces_ifad["iter"] == first_train_iter_ifad, ["rep", "logLik"]
].drop_duplicates(subset=["rep"])
...
if len(omitted_ifad) > 0:
    print(f"Omitted {len(omitted_ifad)} final IFAD logLik values below threshold {threshold_ifad:.2f}:")
...
# Filter initial logliks
loglik_values_init = initial_loglik_ifad["logLik"]
q1_init = loglik_values_init.quantile(0.25)
q3_init = loglik_values_init.quantile(0.75)
iqr_init = q3_init - q1_init
threshold_init = max(q1_init - 3 * iqr_init, -10000)

omitted_init = initial_loglik_ifad[initial_loglik_ifad["logLik"] < threshold_init]
if len(omitted_init) > 0:
    print(f"Omitted {len(omitted_init)} initial IFAD logLik values below threshold {threshold_init:.2f}:")
    print(omitted_init[["rep", "logLik"]].to_string(index=False))
    print()

initial_loglik_filtered_ifad = initial_loglik_ifad[initial_loglik_ifad["logLik"] >= threshold_init].copy()
initial_loglik_filtered_ifad["method"] = "IFAD (Initial)"
```
...
final_loglik_combined = pd.concat([
    ...
    initial_loglik_filtered_ifad[["logLik", "method"]]
], ignore_index=True)
```
...
loglik_summary_init = (
    initial_loglik_filtered_ifad["logLik"]
    .agg(["count", "mean", "std", "min", "median", "max"])
    .round(2)
    .to_frame()
    .T
)
loglik_summary_init.columns = ["N", "Mean", "Std", "Min", "Median", "Max"]
print("\nIFAD (Initial):")
print(loglik_summary_init.to_string(index=False))
```
...
(
    ...
    + scale_fill_manual(values={"IF2": "steelblue", "IFAD": "coral", "IFAD (Initial)": "seagreen"})
    + scale_color_manual(values={"IF2": "steelblue", "IFAD": "coral", "IFAD (Initial)": "seagreen"})
)
```
...
## Final parameter density (perturbation scale)

```{python}
# | label: final-params-pert
# | echo: false
last_iter_pert = traces_long_pert.groupby("rep")["iter"].transform("max")
final_params_pert = traces_long_pert[traces_long_pert["iter"] == last_iter_pert].copy()
final_params_pert["method"] = "IF2"

last_iter_pert_ifad = traces_long_pert_ifad.groupby("rep")["iter"].transform("max")
final_params_pert_ifad = traces_long_pert_ifad[traces_long_pert_ifad["iter"] == last_iter_pert_ifad].copy()
final_params_pert_ifad["method"] = "IFAD"

final_params_pert_combined = pd.concat([
    final_params_pert,
    final_params_pert_ifad
], ignore_index=True)
```

```{python}
# | label: plot-param-density-pert
# | fig-width: 10
# | fig-height: 10
# | echo: false
(
    ggplot(final_params_pert_combined, aes(x="param_value", fill="method", color="method"))
    + geom_density(alpha=0.5)
    + facet_wrap("quantity", scales="free")
    + labs(x="estimate (perturbation scale)", title="Final parameter density (perturbation scale)", fill="Method", color="Method")
    + scale_fill_manual(values={"IF2": "steelblue", "IFAD": "coral"})
    + scale_color_manual(values={"IF2": "steelblue", "IFAD": "coral"})
)
```
