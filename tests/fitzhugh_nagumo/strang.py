...
def _valid(params, dt):
    return (
        jnp.all(jnp.isfinite(jnp.asarray(params)))
        & jnp.isfinite(dt)
        & (dt > 0)
        & (params.epsilon > 0)
        & (params.gamma > params.epsilon / 4)
        & (params.sigma >= 0)
        & (params.s == 0)
    )
...
def _unit_linear_moments(dt, params):
    ...
    dtype = jnp.result_type(dt, *params, 1.0)
    epsilon = jnp.asarray(params.epsilon, dtype=dtype)
    a = jnp.array([[0.0, -1.0 / epsilon], [params.gamma, -1.0]], dtype=dtype)
    q = jnp.diag(jnp.array([0.0, 1.0], dtype=dtype))
...
def path_from_innovations(initial_state, innovations, dt, params=REFERENCE_PARAMETERS):
    ...
    if (
        ...
        or innovations.ndim != 2
        or innovations.shape[1] != 2
    ):
        raise ValueError(
            "Require initial_state shape (2,) and innovations shape (N,2)."
        )
    transition, _ = _unit_linear_moments(dt, params)
    dtype = jnp.result_type(
        initial_state, innovations, transition.dtype, dt, *params, 1.0
    )
    ...
    *,
    ...
    def advance_interval(carry, unused):
        ...
        def advance_substep(carry, unused):
            ...
            return (
                _step_with_matrix(x, xi, simulation_dt, params, transition),
                k,
            ), None

        (state, key), _ = jax.lax.scan(
            advance_substep, (state, key), None, length=nstep
        )
