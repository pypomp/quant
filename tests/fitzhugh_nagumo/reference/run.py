...
def parameters(data):
    return strang.FHNParameters(data["epsilon"],data["gamma"],data["beta"],data["sigma"],0.)


def verify_reference_files(directory=DATA, *,
                           sources=DATA, full=True,
                           expected_record_sha256=None):
    directory, sources = Path(directory), Path(sources)
    record_path = directory / "reference_results.json"
    if expected_record_sha256 is not None and sha(record_path) != expected_record_sha256:
    ...
    if full:
        if not records["distribution_ran_this_invocation"]:
            raise ValueError("Full validation requires a full R reference, not fixtures-only smoke")
        if sha(directory / "distribution_samples.json") != records["output_sha256"]["distribution_samples"]:
...
def coupled_checks(directory=DATA):
    ...
    for case in fixtures["scenarios"]:
        ...
        actual = np.asarray(strang.path_from_innovations(case["initial_state"],case["innovations"],case["dt"],p))
        ...
        np.testing.assert_allclose(e,case["experiment_transition"],rtol=2e-13,atol=2e-15)
        a = np.array([[0.,-1/p.epsilon],[p.gamma,-1.]])
        def integrand(t):
            column = expm(a*t)[:,1]*p.sigma
            return np.outer(column,column)
        independent,_ = quad_vec(integrand,0.,case["dt"],epsabs=1e-25,epsrel=3e-13)
        np.testing.assert_allclose(c,independent,rtol=1e-11,atol=1e-22)
        results.append({
            "name":case["name"],"dt":case["dt"],"n_steps":case["n_steps"],
            "parameters":case["parameters"],"initial_state":case["initial_state"],
            "max_absolute_state_error":float(np.max(np.abs(actual-expected))),
            "max_absolute_voltage_error_vs_package":float(np.max(np.abs(actual[:,0]-voltage))),
            "jax_covariance_max_relative_error_vs_quadrature":float(np.max(np.abs((c-independent)/independent))),
            "author_package_covariance_max_absolute_error_vs_quadrature":float(np.max(np.abs(np.asarray(case["author_covariance"])-independent))),
            "author_package_covariance_max_relative_error_vs_quadrature":float(np.max(np.abs((np.asarray(case["author_covariance"])-independent)/independent))),
            "author_experiment_covariance_max_relative_error_vs_quadrature":float(np.max(np.abs((np.asarray(case["experiment_covariance"])-independent)/independent))),
            "passed":True})
    return results
...
def sample_endpoints(key, initial_state, dt, p, *, n_steps, n_paths):
    ...
    initial = jnp.broadcast_to(jnp.asarray(initial_state,dtype=jnp.float64),(n_paths,2))
    def advance(i, carry):
        states,key = carry
...
def distribution_metrics(a,b,number_of_cdf_checks):
    ...
    return {
        "R_mean":a.mean(0).tolist(),"JAX_mean":b.mean(0).tolist(),
        "R_covariance":ca.tolist(),"JAX_covariance":cb.tolist(),
        "mean_difference_combined_mcse":mean_se.tolist(),"mean_difference_z":mean_z.tolist(),
        "covariance_difference_combined_mcse":cov_se.tolist(),"covariance_difference_z":cov_z.tolist(),
        "ks_distance":[float(k.statistic) for k in ks],"ks_pvalue":[float(k.pvalue) for k in ks],
        "cdf_distance_bound":float(cdf_limit),"cdf_family_alpha":family_alpha,
        "moment_z_threshold":5.,"quantile_probabilities":[.05,.25,.5,.75,.95],
        "R_quantiles":np.quantile(a,[.05,.25,.5,.75,.95],axis=0).tolist(),
        "JAX_quantiles":np.quantile(b,[.05,.25,.5,.75,.95],axis=0).tolist(),
        "passed":bool(np.max(np.abs(mean_z))<5 and np.max(np.abs(cov_z))<5
...
def validate(directory=DATA, *,
    ...
    reference = verify_reference_files(
        directory, sources=sources, full=full,
        expected_record_sha256=expected_record_sha256)
    coupled = coupled_checks(directory)
    print(f"Coupled paths passed: {len(coupled)}, max error {max(x['max_absolute_state_error'] for x in coupled):.3g}", flush=True)
    r = json.loads((directory / "distribution_samples.json").read_text()) if full else {"scenarios": []}
    records,samples = [],[]
    for index,case in enumerate(r["scenarios"]):
        seed = 672013+index
        start = time.perf_counter()
        a = np.asarray(case["endpoint_samples"],dtype=float)
        b = np.asarray(sample_endpoints(jax.random.key(seed),np.asarray(case["initial_state"],float),
                                       case["dt"],parameters(case["parameters"]),
                                       n_steps=case["n_steps"],n_paths=case["n_paths"]))
        assert a.shape == b.shape == (case["n_paths"],2)
        assert np.isfinite(a).all() and np.isfinite(b).all()
        metrics = distribution_metrics(a,b,2*len(r["scenarios"]))
        records.append({"name":case["name"],"dt":case["dt"],"horizon":case["horizon"],
                        "parameters":case["parameters"],"initial_state":case["initial_state"],
                        "paths_per_implementation":case["n_paths"],"jax_seed":seed,
                        "elapsed_jax_seconds_including_compile":time.perf_counter()-start,**metrics})
        samples.append({"name":case["name"],"jax_seed":seed,"endpoint_samples":b.tolist()})
        print(f"Distribution dt={case['dt']}: passed={metrics['passed']}, max mean z={max(abs(x) for x in metrics['mean_difference_z']):.3f}, max covariance z={max(abs(x) for x in metrics['covariance_difference_z']):.3f}",flush=True)
    report = {
        "scope": "Strang simulator only; no likelihood, gradient, optimizer, or 2019 experiment reproduction.",
        "coupled_scenarios": coupled, "distribution_scenarios": records,
...
def main():
    ...
    report, samples, reference = validate(
        reference_dir, full=level != 1,
        expected_record_sha256=r_metadata["reference_record_sha256"])
    metadata = run_metadata({
        "kind": "reference", "model": "fitzhugh_nagumo", "RUN_LEVEL": level,
        "validation_scope": "coupled-only smoke" if level == 1 else "full simulator check",
        "scope": report["scope"], "jax_enable_x64": True,
    ...
    metadata.update(
        ...
        source_sha256={name: sha(MODEL / name) for name in
                       ("model.py", "strang.py", "reference/run.py")},
    )

    out_dir = results / ("smoke/cpu" if level == 1 else "cpu")
    out_dir.mkdir(parents=True, exist_ok=True)
    coupled = pd.DataFrame(report["coupled_scenarios"])
    coupled[["name", "dt", "n_steps", "max_absolute_state_error",
             "max_absolute_voltage_error_vs_package",
             "jax_covariance_max_relative_error_vs_quadrature", "passed"]].to_csv(
                 out_dir / "coupled.csv", index=False)
    distributions = report["distribution_scenarios"]
    rows = []
    for case in distributions:
        rows.append({
            "name": case["name"], "dt": case["dt"], "horizon": case["horizon"],
            "n_paths": case["paths_per_implementation"], "jax_seed": case["jax_seed"],
            ...
            "cdf_distance_bound": case["cdf_distance_bound"], "passed": case["passed"],
        })
    pd.DataFrame(rows, columns=[
        "name", "dt", "horizon", "n_paths", "jax_seed", "max_abs_mean_z",
        "max_abs_covariance_z", "max_cdf_distance", "cdf_distance_bound", "passed",
    ]).to_csv(out_dir / "distribution.csv", index=False)
    endpoints = [
        {"name": case["name"], "dt": case["dt"], "path": index, "V": state[0], "U": state[1]}
    ...
    pd.DataFrame([
        ...
        for case in distributions
    ], columns=["stage", "seconds"]).to_csv(out_dir / "timings.csv", index=False)
    (out_dir / "validation.json").write_text(json.dumps(report, indent=2, allow_nan=False) + "\n")
    metadata["output_sha256"] = {
        name: sha(out_dir / name) for name in
        ("coupled.csv", "distribution.csv", "endpoints.csv", "timings.csv", "validation.json")
    }
    (out_dir / "latest.json").write_text(json.dumps(metadata, indent=2, allow_nan=False) + "\n")
    print(f"Wrote {out_dir}: {metadata['run_config']['validation_scope']}; passed={report['all_passed']}")
