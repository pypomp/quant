...
def build_settings_comparison_html(runs):
    ...
    for run in runs.values():
        ...
        if (
            not hw.get("cores")
            and not slurm.get("cpus")
            and any("cpu" in d.lower() for d in devices)
        ):
...
def load_record(directory):
    ...
    if not np.isfinite(values).all():
        return (
            meta,
            None,
            f"Non-finite likelihoods in {directory}; density comparison omitted",
        )
...
def comparison_warning(r_meta, p_meta):
    ...
    mismatch = [
        key
        for key in required
        if key not in r_cfg or key not in p_cfg or r_cfg[key] != p_cfg[key]
    ]
    if mismatch:
