...
cat("R:\n")
if (exists("t.if.box") && exists("t.L.box") && exists("t.box")) {
    r_time <- data.frame(
        method = c("mif", "pfilter", "total"),
        time = c(t.if.box["elapsed"], t.L.box["elapsed"], t.box["elapsed"]),
        row.names = NULL
    )
    print(r_time)
} else {
    cat("Timing variables not found in the loaded .rda file.\n")
}
```
