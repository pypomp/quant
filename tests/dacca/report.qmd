...
- **GPU Model Used for Parameter Estimation (IF2/Train):** NVIDIA RTX 6000 (Blackwell)
...
# Load R pfilter_check logLiks using pyreadr (fallback to Rscript if needed)
try:
    import pyreadr
    r_res = pyreadr.read_r("pfilter_check/R_results/dacca_results_eval.rda")
    key = "L_box" if "L_box" in r_res else list(r_res.keys())[0]
    val = r_res[key]
    vals_flat = val.values.flatten() if hasattr(val, "values") else np.array(val).flatten()
    r_logliks = [float(x) for x in vals_flat if not np.isnan(x)]
except Exception as pyreadr_err:
    import shutil
    rscript_bin = shutil.which("Rscript") or "Rscript"
    try:
        r_code = (
            'load("pfilter_check/R_results/dacca_results_eval.rda"); cat(L_box, sep=",")'
        )
        result = subprocess.run(
            [rscript_bin, "-e", r_code], capture_output=True, text=True, check=True
        )
...
check_df = pd.DataFrame(check_data, columns=["logLik", "source"]).dropna(subset=["logLik"])
```
