...
# Load the frozen R pfilter_check logLiks. This used to read the .rda through
# pyreadr with an Rscript fallback; both needed a local R and a gitignored file.
# See pfilter_check/R_reference/MANIFEST.json for provenance.
try:
    r_logliks = (
        pd.read_csv("pfilter_check/R_reference/pfilter_logliks.csv")["logLik"]
        .dropna()
        .tolist()
    )
except Exception as e:
    print(f"Error loading frozen R pfilter_check results: {e}")
    r_logliks = []
