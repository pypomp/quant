...
def generate_report():
    ...
    for hf in history_files:
        ...
        if valid_logliks:
            times = [
                datetime.datetime.fromisoformat(d["timestamp"]) for d in valid_logliks
            ]
        ...
        for d in data[-5:]:
            ...
            if isinstance(method_times, list) and method_times:
                mt_str = ", ".join(
                    [
                        f"{mt.get('method', 'unk')}: {mt.get('time', 0):.1f}s"
                        for mt in method_times
                    ]
                )
            else:
                mt_str = "N/A"
            ...
            report_md += (
                f"| {ts} | {p_ver} | {j_ver} | {rl} | {et} | {ll} | {mt_str} |\n"
            )
